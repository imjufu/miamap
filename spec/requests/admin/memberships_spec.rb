...
RSpec.describe 'Admin/Memberships', type: :request do
  ...
  within_subdomain 'guillamap' do
    describe 'GET /admin/members/1/memberships/1' do
      let(:action) { get "/admin/members/#{member.id}/memberships/#{membership.id}" }
    ...
    describe 'GET /admin/members/1/memberships/new' do
      let(:action) { get "/admin/members/#{member.id}/memberships/new" }
    ...
    describe 'POST /admin/members/1/memberships' do
      let(:params) do
      ...
      let(:action) do
        post "/admin/members/#{member.id}/memberships", params: { membership: params }
      end
      ...
      context 'when the user is logged in as an admin' do
        ...
        context 'with invalid params' do
          it 'displays an error' do
            post "/admin/members/#{member.id}/memberships", params: {
    ...
    describe 'GET /admin/members/1/memberships/1/edit' do
      let(:action) do
        get "/admin/members/#{member.id}/memberships/#{membership.id}/edit"
      end
    ...
    describe 'PUT/PATCH /admin/members/1/memberships/1' do
      let(:new_membershiped_at) { Time.zone.local(2019, 4, 18, 12, 32, 14) }
      let(:params) do
        { id: membership.id, membershiped_at: new_membershiped_at }
      end
      let(:action) do
        put "/admin/members/#{member.id}/memberships/#{membership.id}", params: {
          membership: params
        }
      end
    ...
    describe 'DELETE /admin/members/1/memberships/1' do
      let(:action) do
        delete "/admin/members/#{member.id}/memberships/#{membership.id}"
      end
