...
RSpec.describe 'AdminRoom::MembershipForms', type: :request do
  ...
  within_subdomain 'guillamap' do
    describe 'GET /admin_room/membership_forms' do
      let(:action) { get '/admin_room/membership_forms' }
    ...
    describe 'GET /admin_room/membership_forms/1' do
      let(:action) { get "/admin_room/membership_forms/#{membership_form.id}" }
    ...
    describe 'GET /admin_room/membership_forms/new' do
      let(:action) { get '/admin_room/membership_forms/new' }
    ...
    describe 'POST /admin_room/membership_forms' do
      let(:params) { FactoryBot.attributes_for(:membership_form) }
      let(:action) do
        post '/admin_room/membership_forms', params: { membership_form: params }
      end
      ...
      context 'when the user is logged in as an admin' do
        ...
        it 'redirects to the membership form page' do
          action
          expect(response).to redirect_to(admin_room_membership_form_path(MembershipForm.last))
        end

        context 'with invalid params' do
          it 'displays an error' do
            post '/admin_room/membership_forms', params: { membership_form: { title: '' } }
    ...
    describe 'GET /admin_room/membership_forms/1/edit' do
      before { membership_form }

      let(:action) { get "/admin_room/membership_forms/#{membership_form.id}/edit" }
    ...
    describe 'PUT/PATCH /admin_room/membership_forms/1' do
      let(:params) { { id: membership_form.id, title: 'Adhésion 2019' } }
      let(:action) do
        put "/admin_room/membership_forms/#{membership_form.id}",
      ...
      context 'when the user is logged in as an admin' do
        ...
        it 'redirects to the membership form page' do
          action
          expect(response).to redirect_to(admin_room_membership_form_path(membership_form))
        end
      end
    end

    describe 'DELETE /admin_room/membership_forms/1' do
      let(:action) { delete "/admin_room/membership_forms/#{membership_form.id}" }
