  ...
  class ContractsController < AdminRoomController
    ...
    def create
      ...
      respond_to do |format|
        if @contract.save
          format.html { redirect_to admin_room_producer_contract_path(id: @contract.id, producer_id: @producer.id), notice: t('.created') }
    ...
    def update
      respond_to do |format|
        if @contract.update(contract_params)
          format.html { redirect_to admin_room_producer_contract_path(id: @contract.id, producer_id: @producer.id), notice: t('.updated') }
    ...
    def destroy
      @contract.destroy
      respond_to do |format|
        format.html { redirect_to admin_room_producer_path(@producer), notice: t('.destroyed') }
