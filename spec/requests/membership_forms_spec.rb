...
RSpec.describe 'Users', type: :request do
  ...
  within_subdomain 'guillamap' do
    describe 'GET /membership_forms' do
      let(:action) { get '/membership_forms' }
    ...
    describe 'GET /membership_forms/1' do
      let(:action) { get "/membership_forms/#{membership_form.id}" }
    ...
    describe 'GET /membership_forms/new' do
      let(:action) { get '/membership_forms/new' }
    ...
    describe 'POST /membership_forms' do
      let(:params) { FactoryBot.attributes_for(:membership_form) }
      let(:action) do
        post '/membership_forms', params: { membership_form: params }
      end
      ...
      context 'when the user is logged in as an admin' do
        ...
        it 'redirects to the membership form page' do
          action
          expect(response).to redirect_to(MembershipForm.last)
        end

        context 'with invalid params' do
          it 'displays an error' do
            post '/membership_forms', params: { membership_form: { title: '' } }
    ...
    describe 'GET /membership_forms/1/edit' do
      before { membership_form }

      let(:action) { get "/membership_forms/#{membership_form.id}/edit" }
    ...
    describe 'PUT/PATCH /membership_forms/1' do
      let(:params) { { id: membership_form.id, title: 'Adhésion 2019' } }
      let(:action) do
        put "/membership_forms/#{membership_form.id}",
      ...
      context 'when the user is logged in as an admin' do
        ...
        it 'redirects to the membership form page' do
          action
          expect(response).to redirect_to(membership_form)
        end
      end
    end

    describe 'DELETE /membership_forms/1' do
      let(:action) { delete "/membership_forms/#{membership_form.id}" }
