...
RSpec.describe 'Payments', type: :request do
  ...
  within_subdomain 'guillamap' do
    describe 'GET /subscriptions/1/payments/new' do
      let(:action) { get "/subscriptions/#{subscription.id}/payments/new" }
    ...
    describe 'POST /subscriptions/1/payments' do
      let(:params) { FactoryBot.attributes_for(:payment) }
      let(:action) do
        post "/subscriptions/#{subscription.id}/payments", params: {
          payment: params
        }
      end
    ...
    describe 'GET /subscriptions/1/payments/1/edit' do
      let(:action) do
        get "/subscriptions/#{subscription.id}/payments/#{payment.id}/edit"
      end
    ...
    describe 'PUT/PATCH /subscriptions/1/payments/1' do
      let(:params) { { id: payment.id, amount: 110 } }
      let(:action) do
        put "/subscriptions/#{subscription.id}/payments/#{payment.id}",
    ...
    describe 'DELETE /subscriptions/1/payments/1' do
      let(:action) do
