...
RSpec.describe 'Admin/Payments', type: :request do
  ...
  within_subdomain 'guillamap' do
    describe 'GET /admin/subscriptions/1/payments/new' do
      let(:action) { get "/admin/subscriptions/#{subscription.id}/payments/new" }
    ...
    describe 'POST /admin/subscriptions/1/payments' do
      let(:params) { FactoryBot.attributes_for(:payment) }
      let(:action) do
        post "/admin/subscriptions/#{subscription.id}/payments", params: {
          payment: params
        }
      end
      ...
      context 'when the user is logged in as an admin' do
        ...
        it 'redirects to the payment page' do
          ...
          expect(response).to redirect_to(
            admin_subscription_payment_path(
    ...
    describe 'GET /admin/subscriptions/1/payments/1/edit' do
      let(:action) do
        get "/admin/subscriptions/#{subscription.id}/payments/#{payment.id}/edit"
      end
    ...
    describe 'PUT/PATCH /admin/subscriptions/1/payments/1' do
      let(:params) { { id: payment.id, amount: 110 } }
      let(:action) do
        put "/admin/subscriptions/#{subscription.id}/payments/#{payment.id}",
    ...
    describe 'DELETE /admin/subscriptions/1/payments/1' do
      let(:action) do
        delete "/admin/subscriptions/#{subscription.id}/payments/#{payment.id}"
      end
